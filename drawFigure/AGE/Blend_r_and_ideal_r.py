...
diffT_mae[:,0] = np.arange(1, val_data.shape[0]+1)     # first col is ID
no = 1
...
for T in diffT:
    ...
    t = np.zeros(dy.shape[0])
    b = np.zeros(dy.shape[0])
    for i in range(dy.shape[0]):
        mae[i] = np.mean(np.abs(sorted_dy[:(i+1)]))
        

    var_dy_mae = np.zeros((dy.shape[0], 4))  # ID, dropout_mae, dropout_b, ideal_mae
    var_dy_mae[:,0] = np.arange(1, val_data.shape[0]+1)   #ID
    
    var_dy_mae[:,1] = mae       #dropout_mae
    var_dy_mae[:,2] = 
    var_dy_mae[:,3] = mae
